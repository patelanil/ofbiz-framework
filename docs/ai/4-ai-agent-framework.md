...
- [ ] `./gradlew classes` — zero errors
- [ ] `./gradlew checkstyleMain` — zero violations
- [ ] `grep -r "langchain4j" plugins/ai/src` — zero matches
- [ ] Live agentRun call returns coherent answer
- [ ] AiAgentRun + AiAgentToolCall rows present after run
- [ ] Permission enforcement works
- [ ] getUsageSummary returns correct data
- [ ] Two-turn conversation test passes
